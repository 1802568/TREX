...
function preload(){
  ...
  planeImage = loadImage("Airplane-monochrome-icon-by-Hoeda80-580x386.jpg");
...
function spawnObstacles() {
  if(frameCount % 60 === 0) {
    var obstacle = createSprite(displayWidth/1, displayHeight/5);
    //obstacle.debug = true;
...
function spawnPlane() {
  //write code here to spawn the clouds
  if (frameCount % 200=== 0) {
    var plane= createSprite(0, displayHeight/200000);
    plane.y = Math.round(random(10,100));
    ...
    //adjust the depth
    plane.depth = trex.depth;
    trex.depth = trex.depth + 1;
    
    //add each cloud to the group
    cloudsGroup.add(plane);
  }
